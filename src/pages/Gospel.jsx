const verses = {
  en: [
    ...
    },
    {
      ...
      text: "...but the gift of God is eternal life through Jesus Christ our Lord.",
      body: "Salvation isn’t something we can earn — it’s a free gift that we receive by faith.",
    },
    {
    ...
    },
    {
      ...
      body: "God wants you to be saved and secure. Once saved, you're His child forever.",
    },
  ],
  es: [
    {
    ...
    },
    {
      ...
      body: "La salvación no es algo que podamos ganar — es un regalo gratuito que recibimos por fe.",
    },
    {
    ...
    },
    {
      ...
      body: "Dios quiere que seas salvo y seguro. Una vez salvo, eres Su hijo para siempre.",
    },
  ],
};

export default function Gospel({ lang }) {
  const sections = verses[lang] || verses.en;
  const isEnglish = lang === "en";

  return (
    <div className="gospel-page">
      <header className="page-header gospel-header">
        <p className="section-eyebrow">{isEnglish ? "The Gospel" : "El Evangelio"}</p>
        <h1>
          {isEnglish
            ? "Do You Have a Home in Heaven?"
            : "¿Tienes un hogar en el cielo?"}
        </h1>
        <p>
          {isEnglish
            ? "The Bible shows us how we can know that heaven is our home."
            : "La Biblia nos muestra cómo podemos saber que el cielo es nuestro hogar."}
        </p>
      </header>

      <div className="gospel-steps">
        {sections.map((section, index) => (
          <article className="gospel-step" key={section.title}>
            <div className="step-number" aria-hidden="true">
              {String(index + 1).padStart(2, "0")}
            </div>
            <div className="gospel-step-copy">
              <h2>{section.title}</h2>
              <p className="verse-reference">{section.reference}</p>
      ...
      <section className="prayer-callout" aria-labelledby="prayer-title">
        <p className="section-eyebrow">{isEnglish ? "A Prayer of Faith" : "Una Oración de Fe"}</p>
        <h2 id="prayer-title">
          {isEnglish
        ...
        <p>
          <em>
            {isEnglish
              ? "Dear God, I know I’m a sinner. I believe Jesus died for my sins and rose again. I ask You now to forgive me, save me, and give me a home in Heaven. I trust only in You. In Jesus’ name, Amen."
              : "Querido Dios, sé que soy un pecador. Creo que Jesús murió por mis pecados y resucitó. Te pido ahora que me perdones, me salves y me des un hogar en el cielo. Confío solo en Ti. En el nombre de Jesús, Amén."}
          </em>
        </p>
      </section>
    </div>
